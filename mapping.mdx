...
**Two-day patterns (no DAY_OFF):** When the pattern is exactly two days and neither day is DAY_OFF, prefer **cooldownConstraint**: require at least 1 day rest after the first shift type before working the second (e.g. `starts` = first day’s shifts, `ends` = second day’s shifts, `targets.minDays: 1`). CooldownConstraint does not support DAY_OFF, so two-day patterns that involve day off must stay as patternConstraint.

**ScheduleRows → employee filter:** When the XML lists specific `<ScheduleRows><RowNbr>N</RowNbr>...</ScheduleRows>`, the JSON equivalent should restrict to those employees via `filters.employees.ids` (e.g. RowNbr 1 and 2 → `"ids": ["1", "2"]`). Row numbers in the source data typically correspond to employee ids.
...
**JSON equivalent (patternConstraints):** Two undesired patterns for the weekend: (work, day off) and (day off, work). Use `periods` with a recurrent weekend definition so the pattern is evaluated per weekend. ScheduleRows 1 and 2 → restrict to those employees.
...
**JSON equivalent (cooldownConstraint preferred):** Two-day pattern without DAY_OFF: require at least 1 day rest after day shifts before night shifts. ScheduleRows 1 and 2 → apply to employees 1 and 2.
...
Alternatively, **patternConstraint** (undesired pattern: day shift(s) followed by night shift(s)):
...
**JSON equivalent (cooldownConstraint preferred):** Two-day pattern: require at least 1 day rest after night shifts before day shifts.
...
Alternatively, **patternConstraint** (undesired pattern: night shift(s) followed by day shift(s)):
...
**JSON equivalent (cooldownConstraint preferred):** Two-day pattern: require at least 1 day rest after evening/night on-duty before morning/day.
...
Alternatively, **patternConstraint** (undesired pattern: evening/night on-duty followed by morning/day):
...
**JSON equivalent (cooldownConstraint preferred):** Two-day pattern: require at least 1 day rest after morning/day before evening/night on-duty.
...
Alternatively, **patternConstraint** (undesired pattern: morning/day followed by evening/night on-duty):
...
**JSON equivalent (cooldownConstraint preferred):** Two-day pattern: require at least 1 day rest after evening work before evening/night on-duty.
...
Alternatively, **patternConstraint** (undesired pattern: evening work followed by evening/night on-duty):
...
**JSON equivalent (patternConstraints):** Undesired pattern: day off → work → day off. ScheduleRows 1 and 2 → apply to employees 1 and 2. (Cannot use cooldown: pattern involves DAY_OFF.)
...
**JSON equivalent (patternConstraints):** Undesired pattern: work → day off → work. ScheduleRows 1 and 2 → apply to employees 1 and 2. (Cannot use cooldown: pattern involves DAY_OFF.)
...
**JSON equivalent (patternConstraints):** Undesired pattern: evening shift then two days off. ScheduleRows 1 and 2 → apply to employees 1 and 2.
...
**JSON equivalent (patternConstraints):** Undesired pattern: evening shift then day off. ScheduleRows 1 and 2 → apply to employees 1 and 2. (Cannot use cooldown: pattern involves DAY_OFF.)
...
**JSON equivalent (patternConstraints):** Undesired pattern: two days off then morning shift. ScheduleRows 1 and 2 → apply to employees 1 and 2.
...
**JSON equivalent (patternConstraints):** Undesired pattern starting Sunday: work → morning → evening. ScheduleRows 1 and 2 → apply to employees 1 and 2.
...
**JSON equivalent (patternConstraints):** Undesired pattern starting Sunday: work → day shift → evening. ScheduleRows 1 and 2 → apply to employees 1 and 2.
...
**JSON equivalent (patternConstraints):** Undesired pattern starting Friday: evening then day off (Friday evening shift then Saturday off). Cannot use cooldown (DAY_OFF). ScheduleRows 1 and 2.
...
**JSON equivalent (patternConstraints):** Desired pattern starting Friday: work on Fri–Sun, then work (not day off) on Monday. Use `patternDays` with work for all four days so the solver prefers having Monday worked when Fri–Sat–Sun are worked. ScheduleRows 1 and 2.

```json
{
  "id": "MON_OFF_IF_FRI_SAT_SUN",
  "importance": "MEDIUM",
  "isDesired": true,
  "filters": {
    "employees": { "ids": ["1", "2"] }
  ...
  "patterns": [
    {
      "startDays": { "daysOfWeek": ["FRI"] },
      "patternDays": [
        { "shifts": { "labels": ["workloadCategory:WORK", "workloadCategory:ON_DUTY"] } },
        { "shifts": { "labels": ["workloadCategory:WORK", "workloadCategory:ON_DUTY"] } },
        { "shifts": { "labels": ["workloadCategory:WORK", "workloadCategory:ON_DUTY"] } },
        { "shifts": { "labels": ["workloadCategory:WORK", "workloadCategory:ON_DUTY"] } }
      ]
...
**JSON equivalent (patternConstraints):** Desired pattern starting Friday: day off Friday, then work Sat–Mon. ScheduleRows 1 and 2.
...
**JSON equivalent (patternConstraints):** Undesired pattern: night shift then work (no day off). Encourages a day off after a night cluster. ScheduleRows 1 and 2.

```json
{
  "id": "DAYOFF_AFTER_NIGHT_CLUSTER_NEXT",
  "importance": "MEDIUM",
  "isDesired": false,
  "filters": {
    "employees": { "ids": ["1", "2"] }
...
**JSON equivalent (patternConstraints):** Undesired pattern: night cluster then work (no day off). Encourages a day off after nights. ScheduleRows 1 and 2.

```json
{
  "id": "DAYOFF_AFTER_NIGHT_CLUSTER_PREV",
  "importance": "MEDIUM",
  "isDesired": false,
  "filters": {
    "employees": { "ids": ["1", "2"] }
...
**JSON equivalent (patternConstraints):** Desired pattern: day off → evening → evening → day. ScheduleRows 1 and 2.
...
**JSON equivalent (patternConstraints):** Desired pattern: day off → day → evening → evening. ScheduleRows 1 and 2.
...
**JSON equivalent (patternConstraints):** Undesired pattern: night → day off → night. ScheduleRows 1 and 2. (Cannot use cooldown: pattern involves DAY_OFF.)

```json
{
  "id": "AVOID_NIGHT_OFF_NIGHT",
  "importance": "MEDIUM",
  "isDesired": false,
  "filters": {
    "employees": { "ids": ["1", "2"] }
...
**JSON equivalent (cooldownConstraint or patternConstraints):** Undesired: two consecutive evenings (e.g. Friday–Saturday). Prefer **cooldownConstraint**: at least 1 day rest after evening before next evening. ScheduleRows 1 and 2.

```json
{
  "id": "AVOID_FRI_EVENING_EVENING",
  "importance": "MEDIUM",
  "filters": {
    "employees": { "ids": ["1", "2"] }
  },
  "starts": { "shifts": { "labels": ["EVENING"] } },
  "ends": { "shifts": { "labels": ["EVENING"] } },
  "targets": { "minDays": 1 }
}
```

Alternatively **patternConstraint** (undesired pattern starting Friday: evening, evening):
...
**JSON equivalent (patternConstraints):** Desired pattern starting Friday: three consecutive night shifts. ScheduleRows 1 and 2.

```json
{
  "id": "PREFER_FRI_NIGHT_NIGHT_NIGHT",
  "importance": "MEDIUM",
  "isDesired": true,
  "filters": {
    "employees": { "ids": ["1", "2"] }
...
**JSON equivalent (patternConstraints):** Desired pattern starting Saturday: three consecutive night shifts. ScheduleRows 1 and 2.

```json
{
  "id": "PREFER_FRI_NIGHT_NIGHT_NIGHT2",
  "importance": "MEDIUM",
  "isDesired": true,
  "filters": {
    "employees": { "ids": ["1", "2"] }
...
**JSON equivalent (patternConstraints or consecutiveConstraint):** Desired: two consecutive night shifts. Use **patternConstraint** with (NIGHT, NIGHT), or **consecutiveConstraint** with `targets.min: 2` and `filters.shifts.labels: ["NIGHT"]` to encourage at least 2 nights in a row. ScheduleRows 1 and 2.

```json
{
  "id": "PREFER_NIGHT_NIGHT",
  "importance": "MEDIUM",
  "isDesired": true,
  "filters": {
    "employees": { "ids": ["1", "2"] }
...
**JSON equivalent (patternConstraints):** Desired pattern starting Sunday: three consecutive night shifts. ScheduleRows 1 and 2.

```json
{
  "id": "PREFER_SAT_NIGHT_NIGHT_NIGHT",
  "importance": "MEDIUM",
  "isDesired": true,
  "filters": {
    "employees": { "ids": ["1", "2"] }
...
```json
{
  "id": "PREFER_SAT_NIGHT_NIGHT_NIGHT2",
  "importance": "MEDIUM",
  "isDesired": true,
  "filters": {
    "employees": { "ids": ["1", "2"] }
...
**JSON equivalent (patternConstraints):** Undesired pattern: four consecutive days off (max 3 in a row). ConsecutiveConstraint applies to consecutive *shifts*, not days off; use patternConstraint to forbid 4× DAY_OFF in a row. ScheduleRows 1 → apply to employee 1.

```json
{
  "id": "max_continous_dayoff#3",
  "importance": "STRICT",
  "isDesired": false,
  "filters": {
    "employees": { "ids": ["1"] }
...
**"X consecutive in a row" (max/min N shifts of a type):** XML patterns that limit or encourage how many consecutive shifts of a given type can occur in a row (e.g. max 3 night shifts in a row, min 1 evening in a row) should be implemented with **consecutiveConstraint**, not patternConstraint. Use `targets.max` for maximum consecutive (requires importance STRICT) and `targets.min` for minimum consecutive (target only). Use `filters.shifts` to restrict by shift type.
...
**JSON equivalent (consecutiveConstraint):** Max 7 consecutive ON_CALL shifts in a row. ScheduleRows 1 → apply to employee 1.

```json
{
  "id": "max_continous_dutycall_shifts#7",
  "importance": "STRICT",
  "filters": {
    "employees": { "ids": ["1"] },
    "shifts": { "labels": ["ON_CALL"] }
  },
  "targets": {
    "max": 7
  }
...
**JSON equivalent (consecutiveConstraint):** Max 3 consecutive evening shifts in a row. ScheduleRows 1 → apply to employee 1.

```json
{
  "id": "max_continous_evenings#3",
  "importance": "STRICT",
  "filters": {
    "employees": { "ids": ["1"] },
    "shifts": { "labels": ["EVENING"] }
...
**JSON equivalent (consecutiveConstraint):** Max 3 consecutive night shifts in a row. Use `targets.max` with importance STRICT and filter by night shift labels. ScheduleRows 1 → apply to employee 1.
...
**JSON equivalent (consecutiveConstraint):** Max 1 consecutive ON_DUTY shift in a row (no two ON_DUTY days in a row). ScheduleRows 1 → apply to employee 1.

```json
{
  "id": "max_continous_onduty_shifts#1",
  "importance": "STRICT",
  "filters": {
    "employees": { "ids": ["1"] },
    "shifts": { "labels": ["ON_DUTY"] }
  },
  "targets": {
    "max": 1
  }
...
**JSON equivalent (consecutiveConstraint):** Max 5 consecutive work days (WORK or ON_DUTY) in a row. ScheduleRows 1 → apply to employee 1.

```json
{
  "id": "max_continous_shifts#5",
  "importance": "STRICT",
  "filters": {
    "employees": { "ids": ["1"] },
    "shifts": { "labels": ["WORK", "ON_DUTY"] }
  },
  "targets": {
    "max": 5
  }
...
**JSON equivalent (consecutiveConstraint):** Min 1 consecutive evening shift when working evenings (undesired pattern in XML is “not evening” twice in a row). Use `targets.min` as a target; the solver will try to avoid single isolated evening shifts.

```json
{
  "id": "min_continous_evenings#1",
  "importance": "MEDIUM",
  "filters": {
    "employees": { "ids": ["1"] },
    "shifts": { "labels": ["EVENING"] }
...
**JSON equivalent (consecutiveConstraint):** Min 1 consecutive night shift when working nights (undesired pattern in XML is “not night” twice in a row). Use `targets.min` as a target.

```json
{
  "id": "min_continous_nights#1",
  "importance": "MEDIUM",
  "filters": {
    "employees": { "ids": ["1"] },
    "shifts": { "labels": ["NIGHT"] }

...
```json title="Referring to a custom period using start and end days and time interval" highlight="7-8"
{
  "periods": {
    "customDefinitions": [
      {
...
```json title="Referring to a custom period using start and end days and time interval and applying time to all days" highlight="9"
{
  "periods": {
    "customDefinitions": [
      {
...
If you want to create periods that are all the same length, the `recurrentDefintion` field might suit your needs. For example, if you want to create a constraint that applies on a weekly basis, the recurrent definition allows you to
...
Recurrent definitions allow for the use of start and end time similarly to custom definitions. This example shows a recurrent defintion for weekend periods that start Friday afternoon and last until Sunday night.
